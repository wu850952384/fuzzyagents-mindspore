...
def get_trainers(env, advers,goods, obs_shape_n, action_shape_n, arglist):
    ...
    for i in range(num_fuzzy):
        ...
        critics_tar[i] = openai_critic(num_fuzzy * obs_shape_n[0], num_fuzzy * action_shape_n[0], arglist).to(
            arglist.device)
        optimizers_a[i] = optim.Adam(actors_cur[i].trainable_params(), arglist.lr_a)
        optimizers_c[i] = optim.Adam(critics_cur[i].trainable_params(), arglist.lr_c)
    ...
    if game_step > arglist.learning_start_step and \
        ...
        for agent_idx, (actor_c, actor_t, critic_c, critic_t, opt_a, opt_c) in \
            ...
            def forward_fn_c():
                ...
                loss = criterion(q, tar_value)
                return loss, q

            grad_fn_c = ms.ops.value_and_grad(forward_fn_c, None, opt_c.parameters, has_aux=True)

            def train_step_c():
                (loss, _), grads = grad_fn_c()
                opt_c(grads)
                return loss
            ...
            grad_fn_a = ms.ops.value_and_grad(forward_fn_a, None, opt_a.parameters, has_aux=True)

            def train_step_a():
                (loss, _), grads = grad_fn_a()
                opt_a(grads)
                return loss
